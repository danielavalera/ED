...
public:
  ...
  TADLista(int initial_capacity = DEFAULT_CAPACITY)
    : num_elems(0), capacity(initial_capacity), elems(new std::string[capacity]) { }

  ~TADLista() { delete[] elems; }
...
void TADLista::push_front(const std::string &elem) {
...
void TADLista::push_back(const std::string &elem) {
  if (num_elems == capacity) {
    resize_array(capacity * 2);
  }
...
void TADLista::pop_back() {
  assert (num_elems > 0);
  num_elems--;
}


void TADLista::resize_array(int new_capacity) {
...
void TADLista::display(std::ostream &out) const {
  out << "[";
  if (num_elems > 0) {
    out << elems[0];
    for (int i = 1; i < num_elems; i++) {
