...
void ListLinkedSingle::duplicate() {
    Node* cur = head->next;
    while (cur != nullptr) {
        Node* dupe = new Node; //1
        dupe->value = cur->value; //2
        dupe->next = cur->next; //3
        cur->next = dupe; //4
        cur = cur->next->next; //5

        //Sólo con esta línea es el equivalente a las 1,2 y 3
        //Node* aux = new Node{current-> value, current->next};
        //Y le he hecho foto a otra que es el equivalente a las 1,2,3 y 4 creo
        //current->next = new Node{current->value, current->next};
        //y añadimos: la línea 5

        //Consejos para implementar: primero pensar en si es vacía, si solo tiene un elemento, y ya el resto
    }
}
