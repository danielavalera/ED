...
template <typename T> tResult es_zurdo(const BinTree<T>& tree) {
    if (tree.empty())return { 0,true };
    if (tree.left().empty() && tree.right().empty())return { 1,true };
    else {
        tResult iz = es_zurdo(tree.left());
        tResult der = es_zurdo(tree.right());
        tResult sol;
        sol.numNodos = 1 + (iz.numNodos + der.numNodos);
        sol.zurdo = iz.zurdo && der.zurdo && iz.numNodos > der.numNodos;
