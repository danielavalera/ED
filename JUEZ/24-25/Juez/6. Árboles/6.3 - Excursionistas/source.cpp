...
struct tExcursionistas {
    int equipos;
    int max_excursionistas;
};

tExcursionistas excursionistas(const BinTree<int>& tree) {
    if (tree.empty()) return { 0,0 };

    tExcursionistas iz = excursionistas(tree.left());
    tExcursionistas der = excursionistas(tree.right());

    tExcursionistas sol;

    sol.max_excursionistas = max(iz.max_excursionistas, der.max_excursionistas) + tree.root();
    sol.equipos = iz.equipos + der.equipos;

    if (tree.root() > 0 && iz.equipos == 0 && der.equipos == 0) {
        sol.equipos++;
    }
...
void resuelveCaso() {
    ...
    tExcursionistas resultado = excursionistas(tree);
    cout << resultado.equipos << " " << resultado.max_excursionistas << endl;
}

int main() {
#ifndef DOMJUDGE
    std::ifstream in("excursionistas.txt");
